...
class MobileFaceNetIntermediate(nn.Module):
    def __init__(self, embedding_size=128):
        super(MobileFaceNetIntermediate, self).__init__()
        self.conv1 = ConvBNReLU(3, 64, kernel_size=3, stride=2, padding=1)
        self.dwconv2 = DepthwiseConvBlock(64, stride=1)

        self.blocks = nn.Sequential(
            *[DilatedConvBlock(64, 64,) for _ in range(2)],
            BottleneckBlock(64, 64, expansion_factor=2, stride=2),  # Repeat 1 time
            BottleneckBlock(64, 64, expansion_factor=2, stride=1),  # Repeat 4 times
            ...
            *[BottleneckBlock(128, 128, expansion_factor=2, stride=1) for _ in range(2)],  # Repeat 2 times
